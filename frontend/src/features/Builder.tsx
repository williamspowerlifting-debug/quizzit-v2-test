...
function buildYoutubeSentences(segments: any[]) {
  return segments
    .filter(s => s && typeof s.text === "string" && s.text.trim() && Number.isFinite(s.start) && Number.isFinite(s.duration) && s.duration > 0)
    .map(s => {
      ...
      return {
        ...
        words: words.map((w, i) => ({ ...w, start: start + span * i / count, end: start + span * (i + 1) / count })),
      };
    });
}
  ...
  async function processVideo() {
    ...
    try {
      if (source === "local") {
        ...
        const youtubeSentenceTexts = segments
          .filter((s:any) => s && typeof s.text === "string" && s.text.trim() && Number.isFinite(s.start) && Number.isFinite(s.duration) && s.duration > 0)
        ...
        try {
          ...
          if (Array.isArray(refined.sentences) && refined.sentences.length === sentences.length) {
            sentences = sentences.map((s, i) => ({
              ...s,
              start: refined.sentences[i].start,
              end: refined.sentences[i].end,
            }));
          }
        } catch {
